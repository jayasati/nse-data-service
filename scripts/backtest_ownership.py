"""Validate the Ownership engine: does its cross-sectional accumulation score
RANK forward sector-excess returns at 30/60/90d? Bucketed, net-of-cost, reported
for ALL and the A+B tradeable tier (the honest, executable bar).

Ownership changes are quarterly + point-in-time on the DISCLOSURE date, so the
signal is slow-moving — expect the edge (if any) at the longer horizons. Earns a
composite weight only if higher buckets show higher forward sector-excess,
monotonic + positive, at cost.

    PYTHONPATH=src .venv/bin/python -u scripts/backtest_ownership.py --step 20 --lookback 400
"""
...
def main() -> int:
    ...
    from nse_data.research.ownership_engine import score_universe
    ...
    for tier in ("ALL", "A+B", "A core", "B tradeable", "C volatile"):
        report(tier)
        print()
    print("(Ownership earns weight only if higher buckets show higher forward sector-excess, "
