...
def main() -> int:
    ...
    cr = card["credibility"]
    line("Credibility (Engine 12)", (f"{cr['score']}  delivery={cr['delivery']} rev_hit={cr['rev_hit']} "
         f"erratic={cr['erratic']} claims={cr['claims']} adj={cr['conversion_adj']}  [context]")
         if cr else "— (insufficient track record)")
