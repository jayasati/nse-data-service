...
async function init() {
  ...
  $("body").innerHTML = r.rows.map((x, i) => {
    const cell = (v) => `<td class="num ${v >= 7 ? "hi" : ""}">${c(v)}</td>`;
    return `<tr><td class="num">${i + 1}</td><td><a href="/research?sym=${x.symbol}" style="color:#60a5fa;text-decoration:none">${x.symbol}</a></td>` +
      `<td class="num"><b>${x.composite}</b></td><td><span class="tier ${tcls(x.tier)}">${(x.tier || "").split(" ")[0]}</span></td>` +
      cell(x.catalyst) + cell(x.positioning) + cell(x.options) + cell(x.structure) +
      cell(x.volume) + cell(x.rel_strength) + cell(x.vol_expansion) +
      `<td class="gap">${(x.data_gaps || "")}</td></tr>`;
  }).join("");
}
init();
